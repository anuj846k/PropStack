...
import { InlineLoader, useLoading } from "@/components/loading-provider";
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      if (result.status === "initiated" || result.status === "queued") {
        ...
        const reason = result.error_message || result.message;
        alert(`Failed: ${reason}`);
      }
    } catch (err) {
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-gray-900">
                {detail?.tenant_name || tenant.tenant_name}
              </h1>
              <p className="text-sm text-gray-500">
                {detail?.unit_number || tenant.unit_number} ·{" "}
                {detail?.property_name || tenant.property_name}
              </p>
            </div>
            <Button
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium">
                        {detail?.tenant_phone || "No phone"}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Mail className="h-4 w-4 text-gray-400" />
                      <span className="text-sm font-medium">
                        {detail?.tenant_email || "No email"}
                      </span>
                    </div>
                ...
                <Card className="shadow-sm border-gray-200/60">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium">
                        Unit {detail?.unit_number || tenant.unit_number}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-bold text-gray-900">
                        ₹
                        {(
                          detail?.rent_amount || tenant.rent_amount
                        ).toLocaleString()}
                        /mo
                      </span>
                      {tenant.is_overdue && (
                        ...
                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200">
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handlePageChange(callsData.page - 1)
                              }
